import { CompleteClaimData, createSignDataForClaim, fetchWitnessListForClaim, hashClaimInfo } from '@reclaimprotocol/crypto-sdk'
...
describe('Reclaim Tests', () => {
	...
	it('should verify a claim', async() => {
		...
		const context = 'some-application-specific-context'
		...
		const signatures = await Promise.all(
			witnesses
				.map(w => w.wallet.signMessage(claimDataStr))
		)
		...
		const witnessesTs = await fetchWitnessListForClaim(
			{
				...
				witnesses: currentEpoch
					.witnesses
					.map(w => ({ id: w.addr, url: w.host })),
				witnessesRequiredForClaim: currentEpoch.minimumWitnessesForClaimCreation,
